...
class NNDataLoader(object):
    '''
        load the train and test data, including the representations for users and items, generated by nuclear norm
        input: the given filenames of train and test data
        return: train_X, train_Y, test_X, test_Y.
        Besides, can print the information of the data
    '''
    ...
    def _get_group_info(self, ufiles, feature_dir):
        '''
            the feature num of each meta-graph based latent features is different by nuclear norm,
            thus we need to compute N from the dimension number saved in the feature file
            besides, we also generate the group indices for W and V used for group_lasso.
        '''
        N = 0
        gids = [N]
        for filename in ufiles:
            line = open(feature_dir + filename, 'r').readline()
            fnum = int(line.strip().replace('#', ''))
            N += fnum
            gids.append(N)
        N *= 2
        self.config['N'] = N
        self.config['gids'] = gids
        return N, gids
    ...
    def _load_representation(self):
        '''
            load user and item latent features generate by nuclear norm for every meta-graph
        '''
        nnl = self.config.get('nnl')
        if nnl > 0:
            feature_dir = self.data_dir + 'nn_features/path_count/%s/' % nnl
        else:
            feature_dir = self.data_dir + 'nn_features/path_count/'

        ufiles, vfiles = self._generate_feature_files()
        self.N, self.gids = self._get_group_info(ufiles, feature_dir)
        fnum = self.N / 2
        ufilename = self.data_dir + 'uids.txt'
        bfilename = self.data_dir + 'bids.txt'
        uids = [int(l.strip()) for l in open(ufilename, 'r').readlines()]
        ...
        ubase = 0
        for find, filename in enumerate(ufiles):
            ufs = np.loadtxt(feature_dir + filename, dtype=np.float64, comments='#')
            fn = ufs.shape[1] - 1
            for uf in ufs:
                uid = int(uf[0])
                f = uf[1:]
                uid2reps[uid][ubase:ubase+fn] = f
            ubase += fn
