#coding=utf8
'''
    standard fm,with non-convex regularizer, log-sum-penalty.
    The solver is from:
    Quanming Yao, James T. Kwok. Efficient Learning with a Family of Nonconvex Regularizers by Redistributing Nonconvexity.ICML2016
'''
import time
import logging
...
class FMNCR(object):
    ...
    def _lsp(self, x):
        return np.log(1 + x / self.theta)

    def _lsp_glasso(self, G, g_inds):
        '''
            non-convex regularizer of the group lasso. We use LSP.
        '''
        res = 0.0
        for i in range(g_inds.shape[0]):
            res += self._lsp(norm(G[g_inds[i]]))
        return res

    def _obj(self, err, W, P):
        part1 = np.power(err, 2).sum() / self.M
        part2 = self.reg_W * self._lsp_glasso(W, self.gw_inds)
        part3 = self.reg_P * self._lsp_glasso(P.flatten(), self.gp_inds)
    ...
    def _get_updated_paras(self, eta, W, P):
        ...
        for i in range(self.gw_inds.shape[0]):
            grad_W[self.gw_inds[i]] -= self.reg_W / self.theta / (self.theta + norm(W[self.gw_inds[i]])) * W[self.gw_inds[i]]
        ...
        ps = P.shape
        grad_P = grad_P.flatten()
        P = P.flatten()
        for i in range(self.gp_inds.shape[0]):
            grad_P[self.gp_inds[i]] -= self.reg_P * P[self.gp_inds[i]] / self.theta / (self.theta + norm(P[self.gp_inds[i]]))
        grad_P = grad_P.reshape(ps)
        P = P.reshape(ps)
