...
'''
    ...
    latent features is from nuclear norm
'''
import time
import logging
...
class FMNNGL(object):
    ...
    def _init_config(self):
        ...
        self.gids = self.config.get('gids')
    ...
    def _prox_op(self, eta, G, g_inds):
        for i in range(len(g_inds)):
            G[g_inds[i]] = stf(eta * 10.0 / len(g_inds[i]), norm(G[g_inds[i]])) * G[g_inds[i]]
        return G

    def _group_lasso(self, G, g_inds):
        '''
            the reg should be set according to the number of groups
        '''
        res = 0.0
        for i in range(len(g_inds)):
            res += norm(G[g_inds[i]]) * 10.0 / len(g_inds[i])
        return res
    ...
    def _generate_wp_gids(self):
        '''
            generate the gw_inds and gp_inds from gids, which specify the staring ind of the concatenated user features
        '''
        gw_inds, gp_inds = [], []
        for i in range(len(self.gids) - 1):
            gw_inds.append(range(self.gids[i], self.gids[i+1]))
            gw_inds.append(range(self.gids[i] + self.N / 2, self.gids[i+1] + self.N / 2))
            gp_inds.append(range(self.gids[i] * self.K, self.gids[i+1] * self.K))
            gp_inds.append(range((self.gids[i] + self.N / 2) * self.K, (self.gids[i+1] + self.N / 2) * self.K))
        gw_inds = sorted(gw_inds, key=lambda d:d[0])
        gp_inds = sorted(gp_inds, key=lambda d:d[0])
        self.gw_inds = gw_inds
        self.gp_inds = gp_inds

    def train(self):

        W = np.random.rand(self.N) * self.initial # 1 by N
        P = np.random.rand(self.N, self.K) * self.initial# N by K
        self._generate_wp_gids()
