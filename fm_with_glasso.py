...
def get_grad_v_pointwise(err, W, V, X, part, X_square, N, K, M):
    '''
        calculate v pointwise, for checking the correctness of gradients
    '''
    grad_v = np.random.rand(N, K)
    for i in range(N):
        #tmp = np.square(X[:,i])
        for f in range(K):
            ...
            grad_v[i,f] = 0.0
            for j in range(M):
                grad_v[i,f] += 2 * err[j] * (X[j,i] * part[j,f] - V[i,f] * X_square[j,i])
    return grad_v
...
def line_search(err, W, V, b, lamb, eta, gw_inds, gv_inds, obj_v, X, Y, ln, N, K):
    ...
    #grad_v_2 = get_grad_v_pointwise(err, W, V, X, part, X_square, N, K, M)
    #comp_res = np.allclose(grad_v, grad_v_2)
    #logger.info('comparing results of grad_v=%s', comp_res)
